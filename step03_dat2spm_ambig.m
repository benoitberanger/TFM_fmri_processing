...
onsets_datfile = gfile(onsets_path,'00\d_bias.dat$');
...
for iSubj = 1 : length(onsets_datfile)
    ...
    for iRun = 1 : size(onsets_datfile{iSubj},1)
        ...
        if iRun < 6
            dat = importfile_dat_INDIVIEW_bias( deblank(onsets_datfile{iSubj}(iRun,:)) );
        elseif iRun == 6
        ...
        for evt = 1 : size(dat,1)
            ...
            pmod(mvt_noamb).name {3}  = 'ambig';
            pmod(mvt_noamb).param{3}  = dat.diff_ang(flag_NOAMBIG);
            pmod(mvt_noamb).poly {3}  = 1;
            
        end
        
        [pathstr, name, ext] = fileparts(   deblank(onsets_datfile{iSubj}(iRun,:)) );
        save( fullfile(pathstr,[name '_spm_ambig']) , 'names', 'onsets', 'durations', 'pmod' )
